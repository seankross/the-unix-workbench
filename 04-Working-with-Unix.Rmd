...
directory whose names start with "2017", I could do the following:
...
Besides characters that can represent other characters, there are also 
metacharacters called **quantifiers**, which allow you to specify the number of times
a particular regular expression should appear in a string.
One of the most basic quantifiers is `"+"` (plus), which represents one 
...
specific quantifiers you can use beyond "zero or more" or "one or more"
...
the `\d` metacharacter corresponds to all "digit" (number) characters, and the `\s`
...
word without any vowel letters that I could think of).
...
match the beginnings and ends of lines when coupled with other regular
...
"power" (`^`) is used for the beginning of a line, while "money" (`$`) is used for
the end of a line.
...
I know there are many metacharacters to keep track of here, so below I've included
...
use the `-name` flag. The `-name` flag itself then takes an argument, namely the name
...
2. Make five text files with the names of states that don't contain one of
...
command line is the equivalent of entering `cd ~/Documents` into the command
...
we've downloaded is genuine. One way we can prevent malicious
...
To compute the MD5 hash of a file, we can use the `md5` command (or `md5sum` on Ubuntu
for Windows):
...
up a `makefile` that creates a `readme.txt` file automatically
...
file that is created as the result of the **command(s)**. It's very important
...
this section is meant to be just an introduction. For more in-depth reading

...
current computing environment is stored. Evironmental variable names use all
...
The `PATH` variable contains a sequence of paths on our computer seperated by
...
in the directory that are executable can be used as commands. We need to modidy
`PATH` every time we start a shell, so we can ammed our `~/.bash_profile` so

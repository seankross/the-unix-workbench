...
little programming language, and in this chapter we're going to discuss how you can
...
you to make **comments** in your code, i.e., to annotate code so that another 
human being who is reading your code can understand how your program is designed 
to function.
...
The arithmetic operators that you're already familiar with for addition (`+`), 
subtraction (`-`), and multiplication (`*`) work like you would expect them to. 
Notice that, when doing multiplication, you need to escape the star character; otherwise
...
The other numerical operator you should be aware of but that you might not be
...
Notice that, when one number is completely divisible by another number, the
...
If you follow those rules, you can avoid accidentally overwriting data
...
that will be stored in that variable is on the right-hand side of the equals
...
You can modify the value of a variable using arithmetic operators in conjunction 
...
the command in a dollar sign and adding parentheses (`$( )`) around a command.
...
`$2`, etc. An array of all of the arguments passed to your script is stored
...
integer that indicates whether the program was executed successfully or if
an error occurred.
...
As you can see, `true` has an exit status of 0, whereas `false` has an exit status of
...
print "t" if it's true or "f" if it's false:
...
That is a little trick you can use to quickly look at the resulting value of a
logical expression.
...
is no file in this directory called `7`. There are several other varieties of
...
expression is true, then it will execute the code in between `then` and `fi`,
...
think will happen when we make the conditional expression false?
...
condition in the ELIF statement is true, since 5 is greater than 3, so that the
...
can use to control what commands get executed based on their exit status.
...
elements this way. For instance, the fourth element:
...
You can also change an individual element in the array by specifying its
...
- Arrays are linear data structures with ordered elements that can be stored
in variables.
- Each element of an array has an index, and the first index is 0.
...
- Braces allow you to create string sequences and expansions.
...
value 2. The string `"i is equal to 2"` is printed to the console, 
then the loop goes back to the `do` statement since there are still elements
left in the sequence. The `$i` variable is now equal to 3, so `"i is equal to 3"`
...
loops: typing out an explicit list, using an array, or getting the result of a
command substitution. In each case, a variable name is declared after the `for`,
and the value of that variable changes through each iteration of the loop until
...
an iteration of the loop begins, then the code within the WHILE loop will 
continue to be executed. Based on the code for `whileloop.sh`, what do you think
will be printed to the console when we run this script? Let's find out:
...
it is going to print. If the result is different from your prediction,
...
command three times should be the equivalent to using `echo "Hello"` three
...
this function? First, we would need a way to capture a list of arguments that
can have variable length; second, we would need a way to iterate through that
list, so we could add up each element; and third, we would need a way to store the
cumulative sum of the sequence. These three requirements can be satisfied by
using the `$@` variable, a FOR loop, and a variable where we can store the sum.
It's important to break down a larger goal into a series of individual
components before writing a program. That way, we can more easily identify which
...
Many other commands, like `pwd` for example, return a value without affecting
the state of the file on our computer.
There are, however, functions like `mv` or `cp` that move and
...
1. Write a function called `plier` that multiplies together a sequence of
numbers.
2. Write a function called `isiteven` that prints `1` if a number is even or
`0` a number is not even.
3. Write a function called `nevens` that prints the number of even numbers when
provided with a sequence of numbers. Use `isiteven` when writing this function.
4. Write a function called `howodd` that prints the percentage of odd numbers
in a sequence of numbers. Use `nevens` when writing this function.
5. Write a function called `fib` that prints the number of
...
program is going to print to the console, you should decide whether or not your
...
is a file. If any of them were directories, then instead of a hyphen, there would
...
The `chmod` command takes two arguments. The first argument is a string that
...
Let's use `echo` to write a very short program that we'll call `short`.
...
The shebang always begins with `#!`, followed by the path to the program that
...
`PATH` every time we start a shell, so we can ammend our `~/.bash_profile` in order
for our directory for executable scripts to be always in the `PATH`. To modify an
...
4. Write a program called `extremes` that prints the maximum and minimum 

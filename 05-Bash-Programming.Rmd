...
Before the WHILE the `count` variable is set to be equal to 3, but then each
...
re-checked. After three iterations through the loop `count` is equal to 0, so
...
*infinite loop*, so loop never ends and the program runs forever. Obviously we
...
do if we get into a sitation where our program won't terminate.

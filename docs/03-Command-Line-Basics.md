...
expected. Let's add an item to my todo list and then I'll save and exit `nano`
...
- Print a text file to the commdna line using `cat`.
...
make a new directory to store all of my journal entires in called `Journal`. We
...
is the first argument, followed by the patht to the folder where you want the
copy to be made:
...
*recursive*. This ensures that the underlying directorty structure of the
...
to your computer functioning you may cause irreperable damage. I *highly* 
...
line (The path to the Trash Bin on Mac is `~/.Trash`).

TODO: find where trash is on ubuntu
...
TODO: insert obi wan quote about alderaan being blown up
...
Now that the awesome destructive power of `rm` is on your side, you've leared
...
- You should try to aviod using `rm` which permanently removes files or folders.
